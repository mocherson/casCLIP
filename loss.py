...
class ClipLabelLoss(ClipLoss):
    ...
    def __init__(
        ...
        super().__init__(local_loss, gather_with_grad, cache_labels, rank, world_size, use_horovod)

    def get_ground_truth(self, device, num_logits, target) -> torch.Tensor:
        # calculated ground-truth and cache if enabled

        if self.world_size > 1 and self.local_loss:
            return target[num_logits * self.rank : num_logits * (self.rank+1), :].to(device)
        else:
